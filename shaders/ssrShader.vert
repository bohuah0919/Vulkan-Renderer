...
layout(location = 6) in float ior;
...
layout(location = 8) out vec4 screenTexCoord;
layout(location = 9) out float fragIor;
...
void main() {
    ...
    fragKs = uKs;
    screenTexCoord = ubo.proj * ubo.view * ubo.model * vec4(inPosition, 1.0);
    fragIor = ior;
}